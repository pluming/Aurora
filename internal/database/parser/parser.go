...
// Payload stores client.Reply or error
type Payload struct {
	Data client.Reply
	Err  error
}
...
func ParseBytes(data []byte) ([]client.Reply, error) {
	ch := make(chan *Payload)
	reader := bytes.NewReader(data)
	go parse0(reader, ch)
	var results []client.Reply
...
func ParseOne(data []byte) (client.Reply, error) {
	ch := make(chan *Payload)
	reader := bytes.NewReader(data)
	go parse0(reader, ch)
...
func parseSingleLineReply(msg []byte) (client.Reply, error) {
